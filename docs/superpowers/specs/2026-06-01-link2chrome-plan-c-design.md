...
- **视觉交互（`vision.py`，将被替换）**：现状是截图 + 外部视觉模型（豆包/Seed）→ 坐标 → CDP 派发，含 DPR 坐标校准。**本方案弃用外部视觉模型**：cua 模式改为由调用方的主模型（多模态 Claude/GPT 等）自己看图判断坐标，服务端只保留「截图 + DPR 校准 + CDP 坐标派发」这层。`vision.py` 与所有 `DOUBAO_*` 配置随之移除。
...
| cua 视觉判断 | **由调用方主模型完成（多模态）**，弃用外部视觉模型（豆包/Seed） | 主模型直接看图出坐标，链路更短、无额外 API/Key、与 computer-use 形态一致 |
| Tabbit 调试端口 | **支持 `--remote-debugging-port`（已确认）** | pw 模式 attach 与 launch 双模式均可用 |
...
| `modes/cua.py`（精简自 vision.py） | cua 模式：截图 + DPR 校准 + 坐标原语派发（**不调外部模型**，主模型看图驱动） | MCP 工具族 `browser.cua.*` | ws_manager |
...
- **机制**：**由调用方主模型（多模态 Claude/GPT 等）自己看图判断**，服务端不调任何外部视觉模型。工具职责是「把屏幕给主模型看」+「按主模型给的坐标精确派发」。
  - `browser.cua.screenshot()`：返回截图（含尺寸/DPR/缩放元数据，供主模型换算坐标）。
  - `browser.cua.click(x, y)` / `browser.cua.double_click` / `browser.cua.move(x,y)`：坐标原语，服务端做 DPR 校准后 CDP `Input.*` 派发。
  - `browser.cua.type(text)` / `browser.cua.key(combo)` / `browser.cua.scroll(dx,dy)` / `browser.cua.drag(x1,y1,x2,y2)`。
- **来源**：精简现有 `vision.py` 的「截图 + DPR 校准 + CDP 派发」部分；**删除外部模型调用（豆包/Seed）与 `tool_action_vision` 的模型推理路径**。
- **前置条件**：调用方主模型须为多模态（能看图）。Skill 文档注明：非多模态模型应改用 dom-cdp 或 pw 模式。
- **坐标契约**：截图像素 = CSS 像素 × DPR；服务端在派发前 ÷DPR。元数据里明确返回 DPR 与截图实际宽高，避免主模型坐标系误判。
...
- **双模式**（Tabbit 已确认支持 `--remote-debugging-port`，两种都可用）：
  - *attach*：`PLAYWRIGHT_CDP_URL` 或自动发现 Tabbit/Chrome 的 `--remote-debugging-port`；
  - *launch*：`launchPersistentContext(userDataDir, executablePath=Tabbit)`。
...
- **P1 — 模式形式化**：工具拆分为 `browser.dom.*` / `browser.cua.*` 命名空间 + `set_mode` 会话状态；cua 精简为「截图 + 坐标原语」并**移除 `vision.py` / 豆包模型调用 / `DOUBAO_*` 配置**。验收：两模式工具清晰、互不耦合，回归既有用例。
...
- **cua 前置不满足**：调用方主模型非多模态时，cua 工具返回明确提示，建议改用 dom-cdp 或 pw。

## 九、测试策略

- 单元：`browsers/registry.py` 路径解析、cua 坐标/DPR 校准、模式路由。
...
- **不在服务端外挂视觉模型**（cua 由主模型完成；豆包/Seed/`vision.py`/`DOUBAO_*` 移除）。

## 十一、开放项 / 风险

- MV3 keepalive 在 Tabbit 上的实际表现需实测（alarms 最小间隔、offscreen 支持）。
- cua 依赖调用方主模型多模态能力：不同 Agent/模型对截图坐标的判断精度不同，需在 SKILL.md 给出「先 screenshot 再小步校验」的使用范式，必要时叠加可选网格/标尺辅助截图。
- pw 模式 attach 时若用户日常 Tabbit 未带调试端口启动，则自动回退到 launch 模式（独立 user-data-dir，登录态不共享）——需在文档说明此差异。
