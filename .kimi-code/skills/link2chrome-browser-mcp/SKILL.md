...
|
|- 导航
|  |- 当前 session 内导航 -> browser_navigate(session='task-name', action='goto')
|  |- 在 session 中新建标签页 -> browser_tab(session='task-name', action='new')
|  |- 已有 session 标签页 -> browser_tabs_list(session='task-name')，然后 browser_tab(session='task-name', action='switch')
|
|- 交互
|  |- 单个简单动作 -> action_click / action_fill / action_press_key / action_scroll
|  |- 视觉页面或选择器不稳定页面 -> CUA 截图，然后执行小步 CUA 动作
|  |- 多步骤、动态页面、循环、提取、等待、条件判断 -> 优先 browser_code_run
|
|- 调试
   |- 工具失败或连接状态混乱 -> browser_diagnose()
   |- 标签页或标签组不对 -> browser_session(action='list') 或 browser_tabs_list(session='task-name')
```
...
默认选择：只要同一页面任务预计超过 3 个浏览器动作，或包含滚动加载、批量提取、条件分支、显式等待、重复点击/检查，就优先使用 `browser_code_run`。不要先串联很多 `action_*` / `browser_dom_*` 小工具再切换；小工具主要用于一次性点击、一次性截图、或为 `browser_code_run` 前确认视觉状态。

- `agent` 和 `browser` 对象已预注入 `browser_code_run` 的全局作用域。
- `page` 也已预注入；page 是 Link2Chrome 兼容 facade，用来兼容高频 Playwright Page 写法，不是完整 Playwright Page。
- 变量通过 `globalThis` 的 REPL 上下文持久化：第一次调用 `const tab = ...`，第二次调用可直接使用 `tab`。
- `browser_code_run` 工具参数中的 `session` 是权威权限边界。代码内部不要用另一个名字重新 `browser.nameSession(...)`；如需调用，也必须使用同一个 session 名。
...
模型很容易凭 Playwright 习惯写 `page.evaluate(...)`。在 `browser_code_run` 中可以直接使用预注入的 `page` facade；更稳妥的显式写法是先拿到 tab，再把 `tab.playwright` 命名为 page：
...
`browser_diagnose -> browser_session(action='create' 或 'new_tab') -> browser_code_run(session='task-name') 中读取 API 文档 -> 断言当前 URL -> globalThis.tab = target tab`
...
如果当前 session 已经有活动标签页，也可以从 `page` facade 快速开始：
...
一个任务必须先创建一个 session。Session 不只是 Chrome 标签组，也是 Agent 的浏览器权限边界。
...
- 不带 session 调用页面读取、点击、输入、截图、脚本执行工具
- 猜测 tabId 并直接 switch/close
- 把 unrelated 网站拆成多个 session
- 在 finalize 后继续操作同一个 session
